...
class ReadmePrompts:
    ...
    def get_section_specific_prompt(
        ...
        all_sections: List[Dict[str, Any]] = None
    ) -> str:
        """Get a section-specific prompt based on the section type."""
        base_info = cls.get_base_repo_info(repo_info)
        common_guidelines = cls.get_common_guidelines()
        ...
        section_prompts = {
            ...
            - Keep it clean and simple - no decorations or emojis""",
                "context": cls._get_toc_context(all_sections) if all_sections else ""
            },
            "features": {
                "instructions": """
            ...
            - Include git clone command with the repository URL (if available)
            ...
            "usage": {
                "instructions": """
            - START with how to run the application (e.g., npm start, python app.py)
            ...
            - Keep it factual - no legal interpretation""",
                "context": cls._get_license_context(repo_info)
            }
        }
    ...
    def _get_license_context(repo_info: Dict[str, Any]) -> str:
        ...
        if repo_info.get('license'):
            context += f"- License Type: {repo_info.get('license')}\n"
        if repo_info.get('license_file'):
            context += f"- License File: {repo_info.get('license_file')} (exists in repository)\n"
        else:
            context += "- No license file found in repository root\n"
        return context

    @staticmethod
    def _get_toc_context(all_sections: List[Dict[str, Any]]) -> str:
        """Get table of contents context."""
        if not all_sections:
            return ""

        context = "\n\nSections to include in Table of Contents:\n"
        for section in all_sections:
            if section['name'].lower() != 'table of contents':
                context += f"- {section['name']}\n"
        return context

    @classmethod
    def get_header_prompt(cls, repo_info: Dict[str, Any]) -> str:
        """Get the prompt for generating README header section."""
        base_info = cls.get_base_repo_info(repo_info)
        common_guidelines = cls.get_common_guidelines()

        return f"""
        Create only the header section of a README.md for the repository: {repo_info.get('name', 'Project')}
